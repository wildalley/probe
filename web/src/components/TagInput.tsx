    ...
    <div className="space-y-2">
      ...
      >
        {tags.map((tag) => (
          <span
            key={tag}
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-mono font-semibold border shadow-sm transition-all animate-fade-in ${getTagStyle(
              tag,
              isBlueprint
            )}`}
          >
            <TagIcon className="h-3 w-3 opacity-75 shrink-0" />
            <span>{tag}</span>
            <button
              type="button"
              onClick={() => removeTag(tag)}
              className="p-0.5 rounded-full hover:bg-black/15 transition-colors opacity-70 hover:opacity-100"
              title="删除标签"
            >
              <X className="h-3 w-3" />
            </button>
        ...
        <div className="flex-1 min-w-[120px] flex items-center">
          ...
          {inputValue.trim() && (
            <button
              ...
              className="p-1 rounded bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-mono shrink-0 transition-colors"
            >
