...
export function ServerCard({ node, onSelect, theme = "dark", latestAgentVersion }: ServerCardProps) {
  ...
  const displayedPings = pings.length <= 4 || expandedPings ? pings : pings.slice(0, 3);
  ...
  return (
    ...
    >
      ...
      {/* 5. Ping Targets Monitor List (Dedicated row per target) */}
      <div className={cn("mt-3.5 pt-3 border-t text-xs", isBlueprint ? "border-slate-100" : "border-zinc-800/60")}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5 font-semibold text-xs">
            <Activity className="h-3.5 w-3.5 text-sky-500 shrink-0" />
            <span className={isBlueprint ? "text-slate-700" : "text-zinc-300"}>网络监测</span>
            {pings.length > 0 && (
              <span className={cn("px-1.5 py-0.2 rounded-full text-[10px] font-mono", isBlueprint ? "bg-slate-100 text-slate-500" : "bg-zinc-800 text-zinc-400")}>
                {pings.length}
              </span>
            )}
          </div>

          <div className="flex items-center gap-2 text-11 font-mono">
            {node.is_online && avgLatency !== null ? (
              <div className="flex items-center gap-1.5">
                ...
                <span className={cn("font-bold", latencyQuality.color)}>
                  {avgLatency.toFixed(0)}ms
                </span>
              </div>
            ) : (
              <span className="text-zinc-500 font-sans text-10">离线</span>
            )}
            ...
            {node.is_online && (
              ...
              >
                {avgLoss && avgLoss > 0 ? `丢包 ${avgLoss.toFixed(1)}%` : "零丢包"}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          {pings.length === 0 ? (
            ...
            displayedPings.map((p, idx) => {
              ...
              const latVal = isOnline ? `${p.latency_ms.toFixed(1)} ms` : "--";
              const latBarPct = isOnline ? Math.min(100, Math.max(8, (p.latency_ms / 300) * 100)) : 0;

              let barColor = "bg-emerald-500";
              let latColor = "text-emerald-600 dark:text-emerald-400";
              if (!isOnline) {
                barColor = isBlueprint ? "bg-slate-200" : "bg-zinc-800";
                latColor = "text-zinc-500";
              } else if (p.latency_ms >= 200) {
                barColor = "bg-rose-500";
                latColor = "text-rose-500 dark:text-rose-400";
              } else if (p.latency_ms >= 120) {
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden xs:flex sm:flex items-center w-14 sm:w-16 shrink-0">
                    <div className={cn("h-1.5 w-full rounded-full overflow-hidden", isBlueprint ? "bg-slate-200" : "bg-zinc-800")}>
                      <div
                        className={cn("h-full rounded-full transition-all duration-300", barColor)}
                        style={{ width: `${latBarPct}%` }}
                      />
                    </div>
                  </div>

                  <div className="flex items-center gap-2 shrink-0 text-right font-mono">
                    <span className={cn("font-bold text-xs w-16 text-right", latColor)}>
                      {latVal}
                    </span>

                    <span
                      className={cn(
                        "text-[10px] font-mono px-1.5 py-0.2 rounded border w-13 text-center shrink-0 font-medium",
          ...
          {pings.length > 4 && (
            ...
            >
              {expandedPings ? "收起监测目标 ▴" : `展开其余 ${pings.length - 3} 个监测目标 ▾`}
            </button>
          )}
        </div>
      </div>
