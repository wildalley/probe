...
> **全端深度响应式适配 · 极简轻量 Agent · 纯内存事件分发 Hub · 双主题赛博/蓝图看板 · 财务汇率与多行网络监测**  
...
- **主机卡片独立多行网络监测**：卡片底部全面重构为多行独立探测列表，每个监测目标单行清晰展示状态指示灯、目标名称、可视化微型延迟进度条、精确延迟（0.1ms）与丢包率徽章；超过 4 个目标智能折叠展开，兼顾简洁排版与详实指标。
...
- **主机集中管理**：管理后台提供主机搜索与状态列表，将新增部署、地区线路识别、计费周期、到期日、标签和流量配额集中到独立配置弹窗。
