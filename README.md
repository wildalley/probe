...
- 后端计费模型新增已用流量与网卡实时计数的上行、下行字段，并保证上行与下行之和始终等于用于配额计算的总已用流量。
- 校准基线新增按方向保存的锚点；旧数据库会自动补充字段，旧记录缺少方向锚点时会根据当前计数比例恢复并持久化，网卡计数回退时会重新锚定。
- 保存节点配置后的即时广播与 Agent 后续上报共用同一套流量计算逻辑，避免两条路径产生不同结果。
- 已补充无校准、首次锚定、计数器回退、单方向流量及旧记录迁移等测试用例；本次仅记录代码，**尚未执行测试验证**。
- **后续待办**：前端接入 `bandwidth_used_up`、`bandwidth_used_down`、`bandwidth_live_up`、`bandwidth_live_down`，在主机卡片与详情页展示方向明细，并完成数据库升级、接口兼容与端到端测试。
